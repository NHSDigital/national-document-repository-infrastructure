resource "aws_iam_role_policy_attachment" "github_actions_dev_test" {
  ...
  policy_arn = aws_iam_policy.github_actions_dev_test[0].arn
}

resource "aws_iam_policy" "github_actions_dev_test" {
  count = local.is_dev_test ? 1 : 0
  name  = "${terraform.workspace}-github-actions-policy-dev_test"
  path  = "/"
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
