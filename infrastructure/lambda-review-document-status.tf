module "review-document-status-check-lambda" {
  ...
  resource_id         = module.review-document-status-gateway.gateway_resource_id
  http_methods        = ["GET"]
  ...
  lambda_environment_variables = {
    ...
    DOCUMENT_REVIEW_DYNAMODB_NAME = local.is_production ? "" : module.document_upload_review_dynamodb_table[0].dynamodb_read_policy_document
