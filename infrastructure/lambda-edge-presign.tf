...
resource "aws_cloudwatch_log_metric_filter" "edge_presign_error_filter" {
  ...
  pattern        = "%LambdaError%"
  log_group_name = "arn:aws:logs:eu-west-2:${data.aws_caller_identity.current.account_id}:log-groups:/aws/lambda/us-east-1.${terraform.workspace}_${module.edge-presign-lambda.function_name}"
