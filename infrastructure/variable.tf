...
locals {
  is_sandbox              = contains(["ndra", "ndrc", "ndrd"], terraform.workspace)
  is_force_destroy        = contains(["ndra", "ndrb", "ndrc", "ndrd", "ndr-test"], terraform.workspace)
  api_gateway_subdomain_name = "api-${var.environment}"
  api_gateway_full_domain_name = "api-${var.environment}.${var.certificate_domain}"
}
