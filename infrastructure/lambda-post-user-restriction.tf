    ...
    "Statement" : [
      {
        ...
        "Conditon" : {
          "ArnLike" : {
            "aws:SourceArn" : "arn:aws:cloudwatch:eu-west-2:${data.aws_caller_identity.current.account_id}:alarm:${module.post-user-restriction-lambda-alarms.error_alarm_name}",
            "aws:SourceArn" : "arn:aws:cloudwatch:eu-west-2:${data.aws_caller_identity.current.account_id}:alarm:${module.post-user-restriction-lambda-alarms.memory_alarm_name}",
            "aws:SourceArn" : "arn:aws:cloudwatch:eu-west-2:${data.aws_caller_identity.current.account_id}:alarm:${module.post-user-restriction-lambda-alarms.duration_alarm_name}"
          }
        }
        "Resource" : "*"
      }
